...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        DECLARE t_nuevo_saldo_venta INT DEFAULT 0;

        SET cliente = (SELECT cliente_id FROM ventas WHERE id=NEW.venta_id);
        SET cancel = (SELECT cancelado FROM ventas WHERE id=NEW.venta_id);
        ...
        IF (cancel="0") THEN
            ...
            SET t_nuevo_saldo_venta=t_saldo_venta-NEW.total_nota_credito;
        ELSE
            SET t_nuevo_saldo_nota_credito=t_total_nota_credito+NEW.total_nota_credito;
            SET t_nuevo_saldo_venta=0;
        END IF;
        ...
        IF (NEW.anulacion_venta = TRUE) THEN
            UPDATE ventas SET nota_credito=NEW.no_nota_credito, fecha_nota_credito=NEW.fecha_nota_credito,total_nota_credito=t_nuevo_saldo_nota_credito,anulado=NEW.anulacion_venta,cancelado=TRUE, fecha_anulado=NEW.fecha_nota_credito, observaciones_anulado=NEW.observaciones, correlativo=NEW.correlativo WHERE id=NEW.venta_id;
            UPDATE estado_cuentas  SET total_credito=t_credito-NEW.total_nota_credito WHERE cliente_id=cliente;
        ELSE
            UPDATE ventas SET nota_credito=NEW.no_nota_credito, fecha_nota_credito=NEW.fecha_nota_credito,total_nota_credito=t_nuevo_saldo_nota_credito,saldo_venta=t_nuevo_saldo_venta, correlativo=NEW.correlativo WHERE id=NEW.venta_id;
            UPDATE estado_cuentas  SET total_credito=t_credito-NEW.total_nota_credito WHERE cliente_id=cliente;
...
/*
        ...
        DB::unprepared('CREATE TRIGGER agregar_estado_cuenta_abono AFTER INSERT ON abonos
            ...
            DECLARE t_saldo_credito INT DEFAULT 0;
            DECLARE cliente INT DEFAULT 0;

            SET cliente = (SELECT cliente_id FROM ventas WHERE id=NEW.venta_id);
            SET t_abono= (SELECT total_abono FROM estado_cuentas WHERE cliente_id=cliente);
            SET t_saldo_credito = (SELECT saldo_credito FROM ventas WHERE id=NEW.venta_id);

            SET total=t_saldo_credito-NEW.total_abono;

            IF(total!=0) THEN
                UPDATE ventas SET saldo_credito = (t_saldo_credito-NEW.total_abono) WHERE id=NEW.venta_id;
            ELSE
                UPDATE ventas SET saldo_credito = (t_saldo_credito-NEW.total_abono), cancelado=1, fecha_cancelado= NEW.fecha_abono WHERE id=NEW.venta_id;
            END IF;
